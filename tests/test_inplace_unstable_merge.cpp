...
void random_test(const TestParam& param) {
    ...
    for ([[maybe_unused]] int64_t i : std::views::iota(0, param.repeat_count)) {
        ...
        std::ranges::sort(arr.begin(), arr.begin() + param.left_size);
        std::ranges::sort(arr.begin() + param.left_size, arr.end());

        auto expected = arr;
        std::ranges::inplace_merge(expected, expected.begin() + param.left_size);

        try {
            tcs::inplace_unstable_merge::inplace_unstable_merge(arr.begin(), arr.begin() + param.left_size, arr.end());
