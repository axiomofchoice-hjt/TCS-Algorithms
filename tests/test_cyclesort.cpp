...
auto random_test = gen_random_sort_test(
    [](TestArray& arr) { tcs::cyclesort::cyclesort(arr.begin(), arr.end(), IndexedElement::proj); },
    false, kRandomSeed);
...
auto sweep = utest::register_test([] {
    ...
    for (const auto& param : cases) {
        utest::test("cyclesort", "sweep", random_test, param);
    }
});

auto random_cases = utest::register_test([] {
    for (const auto& param : kCases) {
        utest::test("cyclesort", "kCases", random_test, param);
    }
});
}  // namespace
